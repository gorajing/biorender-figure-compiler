...
- **Live BioRender MCP integration with self-rotating tokens.** `src/app/api/resolve-assets/route.ts` calls BioRender's production MCP connector at `https://mcp.services.biorender.com/mcp` over OAuth 2.1 (PKCE + dynamic client registration). Token rotation is handled via Vercel KV: each request reads the current `refresh_token` from KV, exchanges for a fresh access_token + new refresh_token, writes both back atomically, and uses the new access_token for the MCP call. Concurrency-aware retry handles the case where two Lambdas race on refresh. The demo self-rotates indefinitely; no manual token management. The "Resolve via BioRender MCP" button in the figure pane batches one request per unique entity name, displays per-chip status indicators (✓ resolved, · resolved-but-no-placeable, ! error, … loading), and surfaces top match metadata in chip tooltips. `search-icons` tool wired today; `search-templates` is a follow-up.
...
- **Token rotation across cold starts.** SOLVED. Earlier iterations stored the access_token in env and required manual 8-hour rotation. The current implementation uses Vercel KV (Upstash Redis under the hood) as shared state across Lambda invocations: each request reads the current refresh_token from KV, exchanges for a new pair, writes both back atomically, then uses the new access_token for the MCP call. The cache-hit path uses a short-lived in-KV access_token until 5 minutes before expiry, then refreshes. Concurrency-aware retry catches `invalid_grant` and re-reads KV in case another Lambda just rotated. Net result: the demo self-rotates indefinitely without manual intervention.
